...
//				if(row_i == 0){
//				printf("Sending ArrayA value [%d],Column[%d], Total Array index [%d], for row (%d)\n",message->values[i][j],message->columns[i][j], val_i-1, row_i);
//				}
						//vector_X[column],column, value, val_i-1, vector_Y_i);
...
//		for(i=0;i<5;i++){
//			printf("function           THERE ARE %d NZ VALUES IN ROW %d\n",message->row_nz_count[i], i );
//		}
		//if (mutex)
...
int main() {
	...
	while (1) {
		...
		printf("Waiting on PB0 press to generate a random Sparse Matrix...\n");
...
//		int i;
//		for(i=0;i<5;i++){
//			printf("MAIN         THERE ARE %d NZ VALUES IN ROW %d\n",row_NZ_count_A[i], i );
//		}
		...
		calc_output_Y_BOTH_CPUS(columns_A, row_NZ_count_A, values_A,values_count_A_ptr, vector_X, vector_Y_BOTH, message, mutex);
