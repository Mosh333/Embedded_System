...
void LPF_conversion() {
	...
	int Yprime;
	// R_vals[j] = G_vals[j] = B_vals[j] = Y; after this function call
...
void twoDim_Edge_Detect_conversion() {
	...
	// R_vals[j] = G_vals[j] = B_vals[j] = Y; after this function call
	Read_Image_Line(R_vals, G_vals, B_vals);
	memcpy(Y_minus_buf, R_vals, LINE_LEN);
	memcpy(Y, R_vals, LINE_LEN);
	...
	for (i = 0; i < NUM_LINES; i++) {
		if (i == 0) {//for top line
			...
			memcpy(Y_plus_buf, R_vals, LINE_LEN);

		} else if (i < NUM_LINES - 1) {//for common case


			memcpy(Y_minus_buf, Y, LINE_LEN);
			memcpy(Y, Y_plus_buf, LINE_LEN);
			Read_Image_Line(R_vals, G_vals, B_vals);
			memcpy(Y_plus_buf, R_vals, LINE_LEN);

		} else if (i == NUM_LINES - 1) {//for bottom line
			memcpy(Y_minus_buf, Y, LINE_LEN);
			memcpy(Y, Y_plus_buf, LINE_LEN);
			//y plus buf stays same

		}
		for (j = 0; j < LINE_LEN; j++) {
			//if first column
			if (j == 0) {
				Gx = Y_minus_buf[j] - Y_minus_buf[j + 1] + 2 * Y[j] - 2 * Y[j
						+ 1] + Y_plus_buf[j] - Y_plus_buf[j + 1];
			} else if (j == LINE_LEN - 1) {//if last column

				Gx = Y_minus_buf[j - 1] - Y_minus_buf[j] + 2 * Y[j - 1] - 2
						* Y[j] + Y_plus_buf[j - 1] - Y_plus_buf[j];
			} else {
...
int main() {
	...
	while (1) {
		if (filter_flag == 1) {
			printf("Now starting grayscale conversion\n\n");
			Switch_Nios_Mode();
			Y_conversion();
			Switch_HW_Mode();
			filter_flag = 0;
			...
			printf("filter flag is TWO, calling LPF function now\n\n");
			Switch_Nios_Mode();
			LPF_conversion();
		...
		if (filter_flag == 3) {
			printf("Filter flag is three. We will start 2D edge detect now. \nNOTE:We will first run the Y conversion function to convert to Grayscale!\n");
			Switch_Nios_Mode();
			Y_conversion();
			Switch_HW_Mode();
			printf("\nCalling the two_Dim_Edge_Detect_conversion function now!\n\n");
			Switch_Nios_Mode();
			twoDim_Edge_Detect_conversion();

			Switch_HW_Mode();
			filter_flag = 0;
			printf("2D Edge detect conversion complete\n\n");
			//printf("filter flag is %d", filter_flag);
		}
