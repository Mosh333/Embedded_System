...
extern OS_EVENT *LCDMutex;
extern OS_EVENT *SwitchGrpAMutex;
extern OS_EVENT *SwitchGrpBMutex;
extern OS_EVENT *RedLEDMutex;
extern OS_EVENT *GreenLEDMutex; //extern lets us use a variable initialize somewhere elses
...
int main(void) {
	...
	printf("Start main, starting exercise3...\n");
	...
	IOWR(LED_GREEN_O_BASE, 0, 0x0); //turn off
...
int init_OS_data_structs(void) {
	...
	//create other mutex here
	SwitchGrpAMutex = OSMutexCreate(SW_GRPA_MUTEX_PRIORITY, &return_code);
	alt_ucosii_check_return_code(return_code);
	SwitchGrpBMutex = OSMutexCreate(SW_GRPB_MUTEX_PRIORITY, &return_code);
	alt_ucosii_check_return_code(return_code);
