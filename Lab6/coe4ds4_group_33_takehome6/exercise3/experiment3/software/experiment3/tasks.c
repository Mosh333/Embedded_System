...
void custom_task_0(void* pdata) {
	...
	while (1) {
		...
		alt_ucosii_check_return_code(return_code);
		printf("GRPA_mutex obtained...\n");

		printf("Waiting for GreenLEDMutex...\n");
		OSMutexPend(GreenLEDMutex, 0, &return_code);
		alt_ucosii_check_return_code(return_code);
		printf("GreenLEDMutex obtained...\n");
	#endif

	int switchVal0 = IORD(SWITCH_GRPA_I_BASE,0);
	IOWR(LED_GREEN_O_BASE, 0, switchVal0);
	custom_delay(time0);
	IOWR(LED_GREEN_O_BASE, 0, 0); //turn off



	#if MY_MUTEX_ENABLE == 1
		printf("Releasing GreenLEDMutex...\n");
		return_code = OSMutexPost(GreenLEDMutex);
		alt_ucosii_check_return_code(return_code);
		printf("GreenLEDMutex released...\n");
...
void custom_task_1(void* pdata) {
	...
	while (1) {
		...
		printf("Waiting for RedLEDMutex...\n");
		OSMutexPend(RedLEDMutex, 0, &return_code);
		alt_ucosii_check_return_code(return_code);
		printf("RedLEDMutex obtained...\n");
	#endif

		int switchVal1 = IORD(SWITCH_GRPA_I_BASE,0);
		IOWR(LED_RED_O_BASE, 0, switchVal1);
		custom_delay(time1);
		IOWR(LED_RED_O_BASE, 0, 0); //turn off



	#if MY_MUTEX_ENABLE == 1
		printf("Releasing RedLEDMutex...\n");
		return_code = OSMutexPost(RedLEDMutex);
		alt_ucosii_check_return_code(return_code);
		printf("RedLEDMutex released...\n");
...
void custom_task_2(void* pdata) {
	...
	#if MY_MUTEX_ENABLE == 1
		printf("Waiting for SwitchGrpBMutex...\n");
		OSMutexPend(SwitchGrpBMutex, 0, &return_code);
		alt_ucosii_check_return_code(return_code);
		printf("SwitchGrpBMutex obtained...\n");

		printf("Waiting for GreenLEDMutex...\n");
		OSMutexPend(GreenLEDMutex, 0, &return_code);
		alt_ucosii_check_return_code(return_code);
		printf("GreenLEDMutex_mutex obtained...\n");
	#endif

	int switchVal2 = IORD(SWITCH_GRPB_I_BASE,0);
	IOWR(LED_GREEN_O_BASE, 0, switchVal2);
	custom_delay(time2);
	IOWR(LED_GREEN_O_BASE, 0, 0); //turn off
		...
		printf("-Releasing SwitchGrpAMutex...\n");
		return_code = OSMutexPost(SwitchGrpBMutex);
		alt_ucosii_check_return_code(return_code);
		printf("SwitchGrpBMutex released...\n");
	#endif
...
void custom_task_3(void* pdata) {
	...
	while (1) {
		...
		OSMutexPend(RedLEDMutex, 0, &return_code);
		alt_ucosii_check_return_code(return_code);
		printf("RedLEDMutex_mutex obtained...\n");
	#endif

	int switchVal3 = IORD(SWITCH_GRPB_I_BASE,0);
	IOWR(LED_RED_O_BASE, 0, switchVal3);
	custom_delay(time3);
	IOWR(LED_RED_O_BASE, 0, 0); //turn off
		...
		printf("-Releasing SwitchGrpBMutex...\n");
		return_code = OSMutexPost(SwitchGrpBMutex);
		alt_ucosii_check_return_code(return_code);
		printf("SwitchGrpBMutex released...\n");
