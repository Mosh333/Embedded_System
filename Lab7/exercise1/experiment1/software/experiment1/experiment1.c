...
extern OS_EVENT *MboxBubbleSortStart[4];
extern OS_EVENT *MboxBubbleSortDone[4];

extern OS_EVENT *MboxGenArrayStart[4];
//2 stages of merge sort 4 arrays >2 arrays >1(final)
//extern OS_EVENT *MboxMergeSortS1Done[2];
//extern OS_EVENT *MboxMergeSortS2Done;

extern OS_EVENT *MboxMergeSortDone;
...
int init_OS_data_structs(void)
{
	...
	for (i = 0; i < 4; i++) {
		MboxBubbleSortStart[i] = OSMboxCreate((void *)0);
		MboxBubbleSortDone[i] = OSMboxCreate((void *)0);

		MboxGenArrayStart[i] = OSMboxCreate((void *)0);
	}
	MboxMergeSortDone= OSMboxCreate((void *)0);

	MemoryPartition = OSMemCreate(&data_array[0][0], 4, 2*ARRAY_SIZE, &return_code);
	alt_ucosii_check_return_code(return_code);
